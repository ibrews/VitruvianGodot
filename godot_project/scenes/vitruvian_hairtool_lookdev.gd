...
func _make_scalp() -> ShaderMaterial:
	# HAIR TOOL build: the cards ARE the crown hair. The scalp dome is reused only as
	# a DARK, FLAT, non-emissive base (tex_strength 0, emit 0) that fills the dark
	# roots showing through the strand-cutout gaps — it must NOT read as a combed
	# dome of its own. The scalp_cap fade (1.65→1.68) keeps it on the crown only so
	# it never blobs over the forehead.
	vit_scalp_mat = ShaderMaterial.new()
	vit_scalp_mat.shader = load("res://scenes/scalp_cap.gdshader") as Shader
	vit_scalp_mat.set_shader_parameter("hair_color", Color(0.05, 0.035, 0.024))
	vit_scalp_mat.set_shader_parameter("strand_atlas", _tex("res://vit_hair_atlas.png"))
	vit_scalp_mat.set_shader_parameter("roughness_val", 0.9)
	vit_scalp_mat.set_shader_parameter("specular_val", 0.05)
	vit_scalp_mat.set_shader_parameter("anisotropy", 0.0)
	vit_scalp_mat.set_shader_parameter("strand_repeats", 52.0)
	vit_scalp_mat.set_shader_parameter("tex_strength", 0.0)
	vit_scalp_mat.set_shader_parameter("part_darken", 0.0)
	vit_scalp_mat.set_shader_parameter("tonal_variation", 0.0)
	vit_scalp_mat.set_shader_parameter("emit", 0.0)
...
func _make_hair() -> ShaderMaterial:
	# HAIR TOOL CARDS: real authored cards carrying a BAKED normal+AO+diffuse atlas
	# (derived from Hair Tool's HairStripDepth strand height map). The normal map
	# fans across each card so the crown catches the side key light — the fix for
	# the old flat-dark helmet.
	if vit_hair_mat == null:
		...
		vit_hair_mat.set_shader_parameter("hair_color", Color(2.0, 1.8, 1.55, 1.0))
		vit_hair_mat.set_shader_parameter("normal_strength", 1.0)
		vit_hair_mat.set_shader_parameter("flip_green", false)
		vit_hair_mat.set_shader_parameter("ao_strength", 0.6)
		vit_hair_mat.set_shader_parameter("roughness_val", 0.76)
		vit_hair_mat.set_shader_parameter("specular_val", 0.25)
		vit_hair_mat.set_shader_parameter("anisotropy_val", 0.4)
		vit_hair_mat.set_shader_parameter("tonal_variation", 0.7)
		vit_hair_mat.set_shader_parameter("tip_lighten", 0.18)
		vit_hair_mat.set_shader_parameter("emit", 0.15)
		vit_hair_mat.set_shader_parameter("density", 1.0)
		vit_hair_mat.set_shader_parameter("scissor", 0.35)
	return vit_hair_mat
